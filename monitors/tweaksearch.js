    ...
    async run(message) {
        ...
        potential.forEach(async (match, index) => {
            setTimeout(async () => {
                ...
                await msg.react('❌');
                msg.awaitReactions((reaction, user) => reaction.emoji.name === '❌' && (user.id === message.author.id || isGuild ? message.guild.members.get(user.id).permissions.has('ADMINISTRATOR') : false), { time: 20000, max: 1 })

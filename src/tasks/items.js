...
module.exports = class extends Task {
  ...
  async check() {
    ...
    if (res.status !== 200)
      throw "The Fortnite items API is down or unreachable.";
    res = await res.json();
    ...
    this.client.users.forEach(async u => {
      ...
      const common = items.filter(name =>
        settings.includes(name.toLowerCase())
      );
      if (common.length)
        u.send(
          `Chief! These items are now available in the item shop. Remember - spend your V-Bucks wisely!\n\`${common.join(
            ", "
          )}\``
        );
    });

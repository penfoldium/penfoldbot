    ...
    async run(message) {
        ...
        potential.forEach(async (match, index) => {
            setTimeout(async () => {
                ...
                await msg.react('❌');
                msg.awaitReactions((reaction, user) => (reaction.emoji.name === '❌' && (user.id === message.author.id || isGuild ? await message.guild.members.fetch(user.id).then(u => u.permissions.has('ADMINISTRATOR')) : false)) && user !== this.client.user, { time: 20000, max: 1 })

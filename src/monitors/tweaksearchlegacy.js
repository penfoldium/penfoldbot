...
module.exports = class extends Monitor {
  constructor(...args) {
    ...
    this.tweaksearchURL = pkg =>
      `https://cydia.saurik.com/api/macciti?query=${pkg}`;
    this.priceAPI = name =>
      `http://cydia.saurik.com/api/ibbignerd?query=${name}`;
  ...
  async run(message) {
    ...
    if (
      ...
      !(isGuild ? message.guild.settings.get("tweaksearchlegacy") : true)
    )
      return;
    ...
    potential.forEach(async (match, index) => {
      setTimeout(async () => {
        ...
        const embed = new MessageEmbed()
          ...
          .setDescription(
            `[Open in Cydia](https://cdn.penfoldium.org/cydia?package=${name})`
          )

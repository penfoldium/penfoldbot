...
module.exports = class extends Command {
  ...
  async list(message) {
    const reminders = this.client.settings
      .get("schedules")
      .filter(t => t.data.user === message.author.id);
    if (!reminders || reminders.length < 1)
      return message.send("I have nothing to remind you about, chief!");
    const ts = new Timestamp("LLL");
    const display = new RichDisplay(
      new MessageEmbed()
        .setColor(this.client.options.config.embedHex)
        .setAuthor(
          `Reminders for ${message.author.tag}`,
  ...
  async delete(message, id) {
    ...
    if (!reminder)
      return message.send("I don't have any reminders with that ID, chief!");
    if (reminder.data.user !== message.author.id)
      return message.send(
        "That's not nice! You can't just mess with someone else's reminders like that!"
      );
  ...
  async add(message, [id, duration, ...text]) {
    ...
    return message.send(
      `Alright, chief! The reminder's ID is \`${
        reminder.id
      }\` - I'll DM you when it's due!${
        message.guild ? " (I will also message you right here)" : ""
      }`
    );
  }

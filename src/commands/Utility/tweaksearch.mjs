...
module.exports = class extends Command {
    ...
    async run(message, [monitor, target]) {
        ...
        if (monitor == 'standard' && target == 'server') {
            ...
            (message.guild.settings.get('tweaksearch')) ? await message.guild.settings.update('tweaksearch', false) : await message.guild.settings.update('tweaksearch', true);
        ...
        if (monitor == 'legacy' && target == 'server') {
            if (!await message.hasAtLeastPermissionLevel(6)) throw `You don't have permissions to use this command. (Server ownership or Manage Server)`;
            (message.guild.settings.get('tweaksearchlegacy')) ? await message.guild.settings.update('tweaksearchlegacy', false) : await message.guild.settings.update('tweaksearchlegacy', true);

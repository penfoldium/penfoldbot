...
module.exports = class extends Command {
  constructor(...args) {
    super(...args, {
      ...
      subcommands: false,
      description:
        "Have the bot disconnect you or someone else from a voice channel",
...
(If you mention yourself, Move Members is not required)`
  ...
  async run(message, [user = message.member]) {
    ...
    if (!channelID && user === message.member)
      throw "You aren't connected to any voice channel, chief!";
    if (!channelID && user !== message.member)
      throw "The mentioned user is not connected to any voice channel.";

    // Disconnect the user
    await user.voice.setChannel(null);

    return message.send(
      `Disconnected ${user.displayName} from voice successfully.`
    );
  }

...
module.exports = class extends Command {
  constructor(...args) {
    ...
    this.customizeResponse(
      "channel",
      "You need to enter a search term, chief!"
    );
  }
  ...
  async run(message, [channel]) {
    ...
    const stats = (id, key) =>
      `https://www.googleapis.com/youtube/v3/channels?part=statistics,brandingSettings&id=${id}&key=${key}`;

    let toSearch = encodeURI(channel);
    let ch = await fetch(name(toSearch, youtubeAPI));
    ch = await ch.json();

    if (ch.pageInfo.totalResults < 1)
      throw `Oh, crumbs! I couldn't find any channel for \`${channel}\`!`;
    ...
    const fullbanner = banner
      ? banner.replace(
          "w1280-fcrop64=1,32b75a57cd48a5a8",
          "w2560-fcrop64=1,00000000ffffffff"
        )
      : null;
    ...
    const embed = new MessageEmbed()
      ...
      .setFooter(
        `Requested by ${message.author.tag} | Join date:`,
  ...
  async init() {
    if (!this.client.options.config.youtubeAPI) {
      this.client.emit(
        "wtf",
        "YouTube API key not provided in the configuration file, disabling the channel command."
      );
      this.disable();
    }
  }

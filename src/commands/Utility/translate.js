...
You can also use the --alt flag in your command to switch to the alternate translation client (translations may be less accurate than the default client)`
    ...
    this.customizeResponse(
      "input",
      `How am I going to translate if you haven't told me anything yet, chief?\n${this.usageText}`
    );
    this.customizeResponse(
      "output",
    ...
    this.customizeResponse(
      "text",
      `Well, thanks for the languages, I guess, but I still need the text.\n${this.usageText}`
    );
  }
  ...
  async run(message, [input, output, text]) {
    ...
    const errorString =
      "language is not supported or your syntax is incorrect.\nUsage: `translate <input> <output> <text>`";
    ...
    const translated = await translate(text, {
      ...
      throw `An error occured during translation: ${err}`;
    });
    ...
    const inputLanguage =
      languages.getCode(input) === "auto"
        ? recognized
        : languages[languages.getCode(input)];
    const pronunciation = translated.pronunciation;

    // Check if Google was able to translate the given text or not
    if (translatedText === text)
      return message.send(
        `Sorry, Google couldn't translate your text. Please try again, it might be in ${recognized}.`
      );

    // Send a file if the translated text or the pronunciation is equal to or greater than 1024 in length
    if (
      translatedText.length >= 1024 ||
      (pronunciation && pronunciation.length >= 1024)
    ) {
      ...
      return message.send(
        "Translation or pronunciation too long for Discord, have a text file instead.",
        {
          files: [
            {
              name: `translation-${Date.now()}.txt`,
    ...
    const embed = new MessageEmbed()
      ...
      .addField(
        `Translation (Input language: ${inputLanguage})`,
        translatedText
      )
      .setFooter(
      ...
      .setThumbnail(
        "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d7/Google_Translate_logo.svg/480px-Google_Translate_logo.svg.png"
      )
      .setTimestamp();

  ...
  async run(message, [app]) {
    let res = await fetch(
      `https://itunes.apple.com/search?term=${encodeURI(
        app
      )}&country=us&entity=software&limit=1`
    );
    res = await res.json();
    if (res.resultCount === 0)
      return message.send("I couldn't find anything, chief.");
    const icons = {
      ...
      update = new Timestamp("LL").display(
        res.results[0].currentVersionReleaseDate
      ),
    ...
    const embed = new MessageEmbed()
      .setAuthor(
        "App Store Search",
        "https://cdn.penfoldium.org/icons/appstore.png"
      )
      ...
      .addField(
        ...
        `${versionrating} :star: (${versionratingcount} ${
          versionratingcount == 1 ? "rating" : "ratings"
        })`,
        true
      )
      .addField(
        "Rating (all versions)",
        `${rating} :star: (${ratingcount} ${
          ratingcount == 1 ? "rating" : "ratings"
        })`,
        true
      )
      .addField(
        `${res.results[0].genres.length == "1" ? "Category" : "Categories"}`,
        res.results[0].genres.join(", "),
        true
      )
      .addField("Age rating", `${age}`, true)
      .addField(
        "Icons",
        `[60x](${icons[60]}) [100x](${icons[100]}) [512x](${icons[512]}) [1024x](${icons[1024]})`,
        true
      );

...
module.exports = class extends Command {
    ...
    async run(message, [song]) {
        ...
        new Lyrics(geniusToken).search(encodeURI(song)).then(r => {
            ...
            const display = new RichDisplay(new MessageEmbed().setAuthor(`Requested by: ${message.author.tag}`, this.client.user.displayAvatarURL({ size: 1024, format: 'png' })).setTitle(`${r.primaryArtist.name} - ${r.title}`).setThumbnail(r.header).setURL(r.url).setColor(this.client.options.config.embedHex));

...
module.exports = class extends Command {
  ...
  async run(message, [term]) {
    ...
    const url = (search, key) =>
      `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${search}&key=${key}&type=video`;
    const stats = (id, key) =>
    ...
    if (!res.items[0])
      throw "Oh, crumbs! I couldn't find any videos for that search!";
    ...
    if (!("noinfo" in message.flagArgs)) {
      ...
      const embed = new MessageEmbed()
        ...
        .setDescription(
          `(Tip: You can add \`--noinfo\` at the end of your message to omit this section)`
        )
        .addField("Views", views.toLocaleString(), true)
        .setFooter(
          `Requested by ${message.author.tag} | Uploaded on`,
      ...
      !isNaN(likes)
        ? embed.addField("Likes", likes.toLocaleString(), true)
        : embed.addField("Ratings", "Disabled", true);
      !isNaN(dislikes)
        ? embed.addField("Dislikes", dislikes.toLocaleString(), true)
        : null;
      !isNaN(comments)
        ? embed.addField("Comments", comments.toLocaleString(), true)
        : embed.addField("Comments", "Disabled", true);
      !isNaN(likes)
        ? embed.addField(
            "Like ratio",
            `${dislikes ? likeRatio.toFixed(2) + "%" : "Infinity"}`,
            true
          )
        : null;
  ...
  async init() {
    if (!this.client.options.config.youtubeAPI) {
      this.client.emit(
        "wtf",
        "YouTube API key not provided in the configuration file, disabling the video command."
      );
      this.disable();
    }
  }

...
module.exports = class extends Command {
  ...
  async run(message, [search]) {
    let res = await fetch(
      `https://dexonline.ro/definitie/${encodeURI(search)}/json`
    );
    res = await res.json();

    const { definitions } = res;
    if (!definitions || !definitions.length)
      throw "No results found for your search. Check your spelling for any typos.";
    ...
    definitions.forEach(def => {
      ...
      const text =
        thing.length > 1024 ? this.shorten(thing, 1020) + " […]" : thing;
    ...
    const display = new RichDisplay(
      new MessageEmbed()
        .setTitle("dexonline.ro search")
        .setURL(`https://dexonline.ro/definitie/${encodeURI(search)}`)
    ...
    final.forEach(def => {
      ...
      display.addPage(e => {
        ...
        e.addField(
          `Source: ${def.source}`,
          `[Go to definition](https://dexonline.ro/definitie/${def.id})`
        );
        return e;
      });
    });

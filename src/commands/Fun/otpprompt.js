...
module.exports = class extends Command {
  constructor(...args) {
    super(...args, {
      ...
      usageDelim: " ",
      extendedHelp:
        'To add a space to a name (example: "Danger Mouse") you can use underscores instead ("Danger_Mouse") and they will be replaced with spaces in the prompt.'
    });
    this.errorMessage =
      "You need to enter two names in order for me to give you a prompt of your OTP!\n(Hint: If you want to use a space for a name, use an underscore instead and I'll replace it with a space in the prompt.)";
  ...
  async run(message, [personA, personB]) {
    ...
    const embed = new MessageEmbed()
      ...
      .setDescription(
        prompts.random().replace(/{A}/g, personA).replace(/{B}/g, personB)
      )
      .setAuthor(
        `Random OTP prompt for ${personA} and ${personB}`,
        this.client.user.displayAvatarURL({ size: 1024, format: "png" })
      )
      .setTimestamp()
      .setFooter(
        `Requested by ${message.author.tag} | Sourced from prompts.neocities.org`,

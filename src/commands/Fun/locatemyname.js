...
module.exports = class extends Command {
  ...
  async run(message, name) {
    const map = `http://www.locatemyname.com/density-map/${encodeURI(
      name
    )}.jpg`;
    ...
    if (!size)
      throw "There is no known data for this name, chief! (Or you have used unsupported characters)";

    const embed = new MessageEmbed()
      .setColor(this.client.options.config.embedHex)
      .setImage(map)
      .setAuthor(
        `World distribution of the name ${name}`,

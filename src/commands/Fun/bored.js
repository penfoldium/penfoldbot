...
module.exports = class extends Command {
  constructor(...args) {
    super(...args, {
      ...
      subcommands: false,
      description:
        "Are you feeling bored? Let the bot give you a random activity, with additional info included",
  ...
  async run(message) {
    ...
    const activity =
      activityDetails.activity.charAt(0).toLowerCase() +
      activityDetails.activity.slice(1);
    const accessibility = Math.floor(100 - activityDetails.accessibility * 100);
    const type =
      activityDetails.type === "diy"
        ? "DIY"
        : activityDetails.type.charAt(0).toUpperCase() +
          activityDetails.type.slice(1);

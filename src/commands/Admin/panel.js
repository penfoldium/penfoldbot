  ...
  async handleChoices(message, collector) {
    ...
    if (choice == 0) {
      const display = new RichDisplay(
        new MessageEmbed().setColor(this.client.options.config.embedHex)
      );
...
Channels: ${
          g.channels.size
        } (${textChannels} text and ${voiceChannels} voice in ${categoryChannels} categories)
...
Created: ${new Timestamp("LL").display(g.createdAt)}`;
      ...
      guildsResolved.forEach(guildInfo =>
        display.addPage(embed =>
          embed.setTitle("Guild Info").setDescription(guildInfo)
        )
      );
      return display.run(collector.message, {
        filter: (reaction, user) => user === message.author
      });
    }
    ...
    // Set status
      ...
      const collector2 = await this.statusMenu.run(collector.message, {
        filter: (reaction, user) => user === message.author
      });
      ...
      if (status === "invisible") {
        ...
        const [response] = await this.askText
          .createPrompt(message, { target: message.author })
          .run("Do you want to edit the current presence text?");
        if (response) {
          const [text] = await this.askPresenceText
            .createPrompt(message, { target: message.author })
            .run("Please write the presence text you want to use!");
    ...
    // Leave servers
      ...
      const [guild] = await this.askGuildID
        .createPrompt(message)
        .run("What server would you like me to leave?");
      if (guild) {
        await this.client.guilds.get(guild.id).leave();
        return message.send(
          `:white_check_mark: Left \`${guild.name}\` successfully.`
        );
      }
    }
  }
  ...
  async init() {
    ...
    this.menu.addOption(
      "Reinitialize status",
      "Run the 'klasaReady' event if the bot loses its presence/status"
    );

...
ID: ${g.id}
Owner: ${g.owner}
Members: ${g.memberCount} (of which ${Math.floor((bots / g.memberCount) * 100)}% bots)
Created: ${g.createdAt}`
            });
        ...
        // Reboot
            ...
            await collector.message.delete();
            await message.send('See you on the other world, chief!');
            process.exit();
        }

        // Leave servers
        else if (choice == 4) {
            const [guild] = await this.askGuildID.createPrompt(message).run('What guild would you like the bot to leave?');
            if (guild) {
                await this.client.guilds.get(guild.id).leave();
                return message.send(guild)
            }
        }
    }
    ...
    async init() {
        ...
        this.statusMenu.addOption("DND", "Set your status to DND");
    }

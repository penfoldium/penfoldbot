...
module.exports = class extends Command {
  constructor(...args) {
    super(...args, {
      aliases: ["setPrefix"],
      cooldown: 5,
      description: "Change the command prefix the bot uses on your server",
      permissionLevel: 0,
      runIn: ["text"],
      usage: "[reset|prefix:str{1,15}]"
    });
  }
  ...
  async run(message, [prefix]) {
    if (!prefix)
      return message.send(
        `The prefix for this guild is \`${message.guild.settings.get(
          "prefix"
        )}\``
      );
  ...
  async reset(message) {
    ...
    return message.send(
      `Switched the guild's prefix back to \`${this.client.options.prefix}\`!`
    );
  }
};
